...
terms_data = [
    # 2023
    {'term_id': str(uuid.uuid4()), 'year': 2023, 'term_number': 1, 'start_date': datetime(2023, 1, 9).date(),
     'end_date': datetime(2023, 4, 7).date()},
    {'term_id': str(uuid.uuid4()), 'year': 2023, 'term_number': 2, 'start_date': datetime(2023, 5, 8).date(),
     'end_date': datetime(2023, 8, 4).date()},
    {'term_id': str(uuid.uuid4()), 'year': 2023, 'term_number': 3, 'start_date': datetime(2023, 9, 4).date(),
     'end_date': datetime(2023, 11, 17).date()},
    # 2024
    {'term_id': str(uuid.uuid4()), 'year': 2024, 'term_number': 1, 'start_date': datetime(2024, 1, 8).date(),
     'end_date': datetime(2024, 4, 5).date()},
    {'term_id': str(uuid.uuid4()), 'year': 2024, 'term_number': 2, 'start_date': datetime(2024, 5, 6).date(),
     'end_date': datetime(2024, 8, 2).date()},
    {'term_id': str(uuid.uuid4()), 'year': 2024, 'term_number': 3, 'start_date': datetime(2024, 9, 2).date(),
     'end_date': datetime(2024, 11, 15).date()},
    # 2025
    {'term_id': str(uuid.uuid4()), 'year': 2025, 'term_number': 1, 'start_date': datetime(2025, 1, 6).date(),
     'end_date': datetime(2025, 4, 4).date()},
    {'term_id': str(uuid.uuid4()), 'year': 2025, 'term_number': 2, 'start_date': datetime(2025, 5, 5).date(),
     'end_date': datetime(2025, 8, 1).date()},
]
...
# ============================================================================
# 13.5. STUDENT TRANSPORT ASSIGNMENT CHANGES (for historical nuance)
# ============================================================================
print("\n🔄 Generating Occasional Transport Assignment Changes...")

# Initial assignments already have effective_date = None; set it
df_student_transport['effective_date'] = pd.to_datetime('2023-01-01').date()

change_data = []
for term_row in df_terms.itertuples():
    for student_id in transport_student_ids:  # Reuse original list for changes
        if random.random() < 0.05:  # 5% chance per term per student
            bus = random.choice(df_buses.to_dict('records'))
            route = random.choice(df_routes.to_dict('records'))
            route_stops = df_route_stops[df_route_stops['route_id'] == route['route_id']].sort_values('stop_order')
            if len(route_stops) >= 2:
                pickup_stop = route_stops.iloc[random.randint(0, len(route_stops) - 1)]
                dropoff_stop = pickup_stop  # Assume same for simplicity
                ...
                change_data.append({
                    ...
                    'dropoff_stop_id': dropoff_stop['stop_id'],
                    'is_active': True,
                    'effective_date': term_row.start_date
                })

# Append changes
if change_data:
    df_changes = pd.DataFrame(change_data)
    df_student_transport = pd.concat([df_student_transport, df_changes], ignore_index=True)

# Deactivate superseded assignments (keep only the latest active per student at any time)
df_student_transport = df_student_transport.sort_values(['student_id', 'effective_date'])
for student_id in df_student_transport['student_id'].unique():
    student_mask = df_student_transport['student_id'] == student_id
    student_assigns = df_student_transport[student_mask].index
    if len(student_assigns) > 1:
        # Deactivate all but the last (most recent)
        df_student_transport.loc[student_assigns[:-1], 'is_active'] = False

print(f"✅ Added {len(change_data)} transport assignment changes")
print(f"✅ Total assignments: {len(df_student_transport)}")

# ============================================================================
# 13.6. STUDENT TRANSPORT LOGS (Historical with timestamps, distances, and nuances)
# ============================================================================
print("\n📍 Generating Student Transport Logs...")

# Haversine distance function for nuance in distance calculations
...
# School coordinates (from original generation)
SCHOOL_LAT = -1.2764
SCHOOL_LON = 36.7689

# Enhance route_stops with distance_to_school_km (one-time)
df_route_stops['distance_to_school_km'] = df_route_stops.apply(
    lambda row: haversine(SCHOOL_LAT, SCHOOL_LON, row['latitude'], row['longitude']), axis=1
)

# OPTIMIZATIONS: Precompute lookups to avoid slow inner DF ops
# 1. Route -> sorted stops (by distance)
route_stops_dict = {}
for route_id in df_routes['route_id'].unique():
    route_stops = df_route_stops[df_route_stops['route_id'] == route_id].copy()
    if len(route_stops) > 0:
        route_stops_sorted = route_stops.sort_values('distance_to_school_km')  # Closer first
        route_stops_dict[route_id] = route_stops_sorted

# 2. Stop_id -> details dict (route_id, distance, time_from_prev)
stop_details = df_route_stops.set_index('stop_id')[['route_id', 'distance_to_school_km', 'time_from_prev_stop_min']].to_dict('index')

# 3. Route_id -> details dict
route_details = df_routes.set_index('route_id')[['total_distance_km', 'estimated_duration_min']].to_dict('index')

# 4. Attendance lookup: dict of (date, student_id) -> status (O(1) access)
attendance_lookup = {}
for _, row in df_attendance.iterrows():
    key = (row['date'], row['student_id'])
    attendance_lookup[key] = row['status']

# Transport logs data
transport_logs_data = []

current_date_cap = datetime(2025, 10, 6).date()  # Historical up to today

for term_row in df_terms.itertuples():
    if term_row.start_date > current_date_cap:
        continue  # Skip future terms
    term_end = min(term_row.end_date, current_date_cap)
    current_date = term_row.start_date
    
    while current_date <= term_end:
        if current_date.weekday() < 5:  # School days only (Mon-Fri)
            
            # Get active assignments for this date: latest per student where effective_date <= current_date and is_active
            date_assignments = df_student_transport[
                (df_student_transport['effective_date'] <= current_date) & 
                df_student_transport['is_active']
            ].copy()
            date_assignments = date_assignments.sort_values(['student_id', 'effective_date'], ascending=[True, False])
            date_assignments = date_assignments.drop_duplicates('student_id', keep='first')
            
            # Filter to active students (admitted and not dropped out)
            active_students = df_students[
                (df_students['date_of_admission'] <= current_date) & 
                ((df_students['dropout_date'].isna()) | (df_students['dropout_date'] > current_date))
            ]['student_id'].tolist()
            date_assignments = date_assignments[date_assignments['student_id'].isin(active_students)]
            
            # Add route_id via lookup (faster than merge)
            date_assignments_with_route = date_assignments.copy()
            date_assignments_with_route['route_id'] = date_assignments_with_route['pickup_stop_id'].map(
                lambda sid: stop_details.get(sid, {}).get('route_id')
            )
            date_assignments_with_route = date_assignments_with_route.dropna(subset=['route_id'])
            date_assignments_with_route = date_assignments_with_route.drop_duplicates(['student_id', 'bus_id', 'pickup_stop_id'])
            
            for _, assign_row in date_assignments_with_route.iterrows():
                student_id = assign_row['student_id']
                bus_id = assign_row['bus_id']
                pickup_stop_id = assign_row['pickup_stop_id']
                route_id = assign_row['route_id']
                archetype_name = student_archetypes[student_id]
                
                # Get stop details via dict (fast)
                stop_det = stop_details.get(pickup_stop_id, {})
                if not stop_det:  # Skip invalid
                    continue
                distance_km = stop_det['distance_to_school_km']
                
                # Get pre-sorted route stops
                if route_id not in route_stops_dict:
                    continue
                route_stops_sorted = route_stops_dict[route_id]
                if pickup_stop_id not in route_stops_sorted['stop_id'].values:
                    continue
                stop_position = route_stops_sorted[route_stops_sorted['stop_id'] == pickup_stop_id].index[0]
                num_stops_route = len(route_stops_sorted)
                
                # Route details via dict
                route_det = route_details.get(route_id, {})
                total_distance_km = route_det.get('total_distance_km', 10)
                estimated_duration_min = route_det.get('estimated_duration_min', 50)
                avg_interval_min = route_stops_sorted['time_from_prev_stop_min'].fillna(8).mean()  # Use sorted for consistency
                
                # Travel speed nuance: 25 km/h average for return trips
                travel_speed_kmh = 25
                estimated_travel_back_min = (total_distance_km / travel_speed_kmh) * 60
                
                # Base times for the day
                morning_start = datetime.combine(current_date, time(6, 0, 0))  # 6:00 AM pickup start
                afternoon_start = datetime.combine(current_date, time(16, 0, 0))  # 4:00 PM school pickup
                base_dropoff_school = morning_start + timedelta(minutes=estimated_travel_back_min + (avg_interval_min * (num_stops_route - 1)))  # Full route time
                # Stagger school arrival by route distance (farther routes arrive ~10-15 min later)
                max_route_distance = df_route_stops['distance_to_school_km'].max()
                stagger_min = (distance_km / max_route_distance) * 15 if max_route_distance > 0 else 0
                scheduled_dropoff_school = base_dropoff_school + timedelta(minutes=stagger_min)
                
                # Afternoon school pickup stagger (slight, for bus sequencing)
                afternoon_stagger_min = (distance_km / max_route_distance) * 5 if max_route_distance > 0 else 0
                scheduled_pickup_school = afternoon_start + timedelta(minutes=afternoon_stagger_min)
                
                # Archetype-based delay distribution (nuance: at_risk more variable/late)
                delay_mean = 0
                delay_std = 5
                if archetype_name == 'at_risk':
                    ...
                    delay_mean = 8
                    delay_std = 8
                elif archetype_name == 'fluctuating':
                    delay_mean = 3
                    delay_std = 10
                else:  # improving, high_performer
                    delay_mean = -3
                    delay_std = 4
                
                # Daily delay for this student (correlate across events, with slight variation)
                base_delay_min = max(-10, min(60, np.random.normal(delay_mean, delay_std)))  # Cap extremes
                morning_delay = base_delay_min + np.random.normal(0, 2)  # Small variation
                afternoon_delay = base_delay_min + np.random.normal(0, 2)
                
                # Event times (actual = scheduled + delay)
                scheduled_pickup = morning_start + timedelta(minutes=avg_interval_min * stop_position)
                actual_pickup = scheduled_pickup + timedelta(minutes=morning_delay)
                
                actual_dropoff_school = scheduled_dropoff_school + timedelta(minutes=morning_delay)  # Travel time constant
                
                actual_pickup_school = scheduled_pickup_school + timedelta(minutes=afternoon_delay / 2)  # Half delay at school
                
                # Afternoon dropoff: +5 min from school to first stop + intervals
                scheduled_dropoff = afternoon_start + timedelta(minutes=5 + avg_interval_min * stop_position)
                actual_dropoff = scheduled_dropoff + timedelta(minutes=afternoon_delay)
                
                # Check attendance via dict lookup (fast!)
                att_key = (current_date, student_id)
                is_absent = False
                att_status = 'present'
                if att_key in attendance_lookup:
                    att_status = attendance_lookup[att_key]
                    if att_status == 'absent':
                        is_absent = True
                    # If 'late' in attendance, amplify transport delay slightly
                    elif att_status == 'late':
                        morning_delay += 5  # Extra for late arrival
                        actual_pickup += timedelta(minutes=5)
                
                # Generate log for each event type
                event_configs = [
                    ('morning_pickup', scheduled_pickup, actual_pickup, pickup_stop_id, 'home_stop'),
                    ('morning_dropoff', scheduled_dropoff_school, actual_dropoff_school, None, 'school'),
                    ('afternoon_pickup', scheduled_pickup_school, actual_pickup_school, None, 'school'),
                    ('afternoon_dropoff', scheduled_dropoff, actual_dropoff, pickup_stop_id, 'home_stop')
                ]
                
                for event_type, scheduled, actual, stop_id_val, location_type in event_configs:
                    notes = None
                    status = None
                    final_actual = actual
                    
                    # Override for absences (no_show only for home events)
                    if is_absent and location_type == 'home_stop':
                        status = 'no_show'
                        final_actual = None
                        notes = f"Class {att_status}: {random.choice(['Family emergency', 'Illness', 'Traffic', 'Other'])}"
                    else:
                        # Calculate status based on delta (nuance: thresholds with archetype adjustment)
                        if final_actual:
                            delta_min = (final_actual - scheduled).total_seconds() / 60.0
                            # Adjust thresholds slightly for archetype
                            late_threshold = 5 + (5 if archetype_name in ['at_risk', 'declining'] else 0)
                            missed_threshold = 30 + (10 if archetype_name == 'at_risk' else 0)
                            early_threshold = -5
                            
                            if delta_min > missed_threshold:
                                status = 'missed'
                                notes = f"Severe delay: {delta_min:.0f} min (traffic overload)"
                            elif delta_min > late_threshold:
                                status = 'late'
                                notes = f"{delta_min:.0f} min late (minor delay)"
                            elif delta_min < early_threshold:
                                status = 'early'
                                notes = f"{abs(delta_min):.0f} min early (smooth run)"
                            else:
                                status = 'on_time'
                                notes = f"On time ({delta_min:.0f} min variance)"
                        else:
                            continue  # Skip if no actual time
                    
                    # Recorded at: shortly after actual (or scheduled if no-show)
                    recorded_at = final_actual + timedelta(hours=1) if final_actual else scheduled + timedelta(hours=1)
                    ...
                    transport_logs_data.append({
                        ...
                        'actual_time': final_actual,
                        'status': status,
                        'location_type': location_type,
                        'notes': notes,
                        'recorded_at': recorded_at,
                        'distance_to_school_km': distance_km  # For analysis
                    })
        
        current_date += timedelta(days=1)

df_student_transport_logs = pd.DataFrame(transport_logs_data)

print(f"✅ Created {len(df_student_transport_logs):,} transport log records")
...
dataframes = {
    ...
    'student_transport_logs.csv': df_student_transport_logs,
...
print(f"   Students Using Transport: {len(df_student_transport[df_student_transport['is_active'] == True])}")

print(f"\n📍 Transport Logs:")
print(f"   Total Logs: {len(df_student_transport_logs):,}")
by_status = df_student_transport_logs['status'].value_counts()
for status, count in by_status.items():
    print(f"   {status}: {count} ({count / len(df_student_transport_logs) * 100:.1f}%)")
